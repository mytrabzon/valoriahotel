...
import { setPendingTransferTourMapPick } from '@/lib/pendingTransferTourMapPick';
...
export default function TransferTourPickLocation() {
  ...
  const mapKey = useMemo(() => `tt-pick-${initialCenter.lat}-${initialCenter.lng}`, [initialCenter.lat, initialCenter.lng]);
  ...
  const confirm = useCallback(async () => {
    ...
    try {
      ...
      setPendingTransferTourMapPick({ lat: c.lat, lng: c.lng, address: `${c.lat.toFixed(5)}, ${c.lng.toFixed(5)}` });
      router.back();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <View style={styles.root}>
      <Text style={styles.hint}>{t('transferTourPickMapScreenHint')}</Text>
...
const styles = StyleSheet.create({
  ...
  crossV: { position: 'absolute', width: 2, height: 28, backgroundColor: 'rgba(30,58,95,0.95)', borderRadius: 1 },
  crossH: { position: 'absolute', width: 28, height: 2, backgroundColor: 'rgba(30,58,95,0.95)', borderRadius: 1 },
