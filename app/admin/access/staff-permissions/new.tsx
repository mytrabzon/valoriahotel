...
export default function NewStaffPermissionScreen() {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const validFromDate = valid_from || null;
      const validUntilDate = valid_until || null;
      const { error } = await supabase.from('staff_door_permissions').insert({
        staff_id,
        door_id,
      ...
      router.replace('/admin/access/staff-permissions');
    } catch (e: unknown) {
      Alert.alert('Hata', (e as Error)?.message ?? 'Yetki eklenemedi.');
    }
    setLoading(false);
  };
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={90}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.label}>Başlangıç saati (HH:mm, boş = sınırsız)</Text>
        <TextInput
          style={styles.input}
          value={time_start}
