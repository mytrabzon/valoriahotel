-- Valoria Hotel - Acil durum: admin personele push gönderilebilmesi için RPC bildirim yapan staff_id listesini döndürür
-- Return type changed from INTEGER to TABLE(staff_id UUID) -> must drop then create (PostgreSQL does not allow changing return type)
DROP FUNCTION IF EXISTS public.create_emergency_alert(UUID, TEXT, TEXT);

CREATE FUNCTION public.create_emergency_alert(
  p_guest_id UUID,
...
COMMENT ON FUNCTION public.create_emergency_alert IS 'Misafir panik butonu: tüm admin hesaplarına acil bildirim yazar ve bildirim gönderilen staff_id listesini döndürür (push için).';
