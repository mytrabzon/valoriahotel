-- Apple (ve diğer OAuth) ile giriş: JWT'de email bazen boş gelir. Bu yüzden auth user id ile
-- misafir eşleştirmesi ekleniyor; böylece "oturum açın" / "giriş yapın" uyarısı alınmaz.
...
COMMENT ON COLUMN public.guests.auth_user_id IS 'Supabase auth.users id; Apple vb. ile girişte e-posta JWT''de olmayabiliyor, bu alanla misafir eşleştirilir.';

-- Çağıran kullanıcı (auth.uid()) için misafir getir veya oluştur; guest_id ve app_token döner.
-- Önce e-posta (JWT) ile dene; yoksa auth_user_id ile bul/oluştur (Apple senaryosu).
...
COMMENT ON FUNCTION public.get_or_create_guest_for_caller(TEXT) IS
  'Çağıran kullanıcı için misafir: önce JWT email ile, yoksa auth.uid() ile bulur/oluşturur; guest_id ve app_token döner. Apple ile girişte JWT email boş olabildiği için auth_user_id kullanılır.';
